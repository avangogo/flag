...
struct
  ...
  let raw_multiply p2 denom v1 v2 =
    Array.map (fun m ->
      F.div
  ...
  let vect_multiply ?p2 ?denom a b =
    let prod_basis = mul_basis a.basis b.basis in 
    let p2 = match p2 with
      | Some x -> x
      | None -> S.get_p2 a.basis b.basis prod_basis in
    let denom = match denom with
      | Some x -> x
      | None -> S.get_p2_denom a.basis b.basis prod_basis
    in
    { name = option_map2 (sprintf "%s*%s") a.name b.name;
      expr = Times (a.expr, b.expr);
      basis = mul_basis a.basis b.basis;
      vect = raw_multiply p2 denom a.vect b.vect }
 ...
 (* Multiplication followed by unlabelling *)
  let vect_multiply_and_unlabel ?p2untype ?denom a b =
    let prod_basis = mul_basis a.basis b.basis in 
    let p2untype = match p2untype with
      | Some x -> x
      | None -> S.get_p2untype a.basis b.basis (untype_basis prod_basis) in
    let denom = match denom with
      | Some x -> x
      | None ->
         (S.get_p2_denom a.basis b.basis prod_basis)*( S.get_q_denom prod_basis)
    in
    { name = option_map2 (sprintf "[|%s*%s|]") a.name b.name;
      expr = Unlab (Times (a.expr, b.expr));
      basis = untype_basis (mul_basis a.basis b.basis);
      vect = raw_multiply p2untype denom a.vect b.vect }

  let multiply_and_unlabel a b = vect_multiply_and_unlabel a b
      
  let multiply_and_unlabel_all t1 t2 =
    let b1 = t1.(0).basis in
    let b2 = t2.(0).basis in
    assert (array_for_all (fun v -> v.basis = b1) t1);
    assert (array_for_all (fun v -> v.basis = b2) t2);
    let b3 = untype_basis (mul_basis b1 b2) in
    let p2untype = S.get_p2untype b1 b2 b3 in
    let denom =
      let b = mul_basis b1 b2 in
      ( S.get_p2_denom b1 b2 b )*( S.get_q_denom b ) in
    let init i j =
      vect_multiply_and_unlabel ~p2untype:p2untype ~denom:denom t1.(i) t2.(j) in
    Array.init (Array.length t1) (fun i -> Array.init (Array.length t2) (init i))
